...
char * split_the_string(char * string_ptr, char delimiter)
{
    char *splitPoint_ptr = NULL;                        //char pointer to hold our second string

    if(string_ptr == NULL)                              //if string_ptr is NULL return error
    {
        return ERROR_NULL_POINTER;
    }
    else if(delimiter == '\0')                          //if delimiter is NULL return error
    {
        return ERROR_NULL_DELIMITER;
    }
    for(int i = 0; i < strlen(string_ptr); i++)         //for every letter in string
    {
        if(*(string_ptr + i) == delimiter)              //if it is equal to the delimeter
        {
            if(splitPoint_ptr == NULL)                  //if another pointer isn't stored then save else return error
            {
                splitPoint_ptr = (string_ptr + i + 1);  //Save the next position. Because the Delimeter will be deleted on Success                
            }
            else
            {
    ...
    if(splitPoint_ptr == NULL)                          //If splitPoint_ptr is null then we didnt find one and return orignal string_ptr
    {
	    return string_ptr;        
    }
    else                                                //else delete the delimeter and return the pointer to the second string
    {
        *(splitPoint_ptr - 1) = '\0';                   //delete the delimeter
        
        return splitPoint_ptr;
    }
}
